...
__device__ float float16BitsToFloatDevice(std::uint16_t bits) {
  unsigned int sign = static_cast<unsigned int>(bits & 0x8000u) << 16u;
  unsigned int exponent = (bits >> 10u) & 0x1fu;
  ...
  if (exponent == 0u) {
    if (mantissa == 0u) {
      ...
      mantissa &= 0x03ffu;
      widened = sign |
                (static_cast<unsigned int>(unbiased_exponent + 127) << 23u) |
                (mantissa << 13u);
    }
...
__global__ void embedFloat16Kernel(const int* tokens,
                                   const std::uint16_t* token_embedding,
                                   const std::uint16_t* position_embedding,
                                   float* x, int seq_len, int dim) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  int channel = idx % dim;
  int token_id = tokens[token_pos];
  x[idx] = float16BitsToFloatDevice(token_embedding[token_id * dim + channel]) +
           float16BitsToFloatDevice(position_embedding[token_pos * dim + channel]);
}
...
__global__ void linearFloat16Kernel(const float* input,
                                    const std::uint16_t* weight,
                                    const std::uint16_t* bias, float* output,
                                    int rows, int in_dim, int out_dim) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  int row = idx / out_dim;
  int out_col = idx % out_dim;
  float sum = float16BitsToFloatDevice(bias[out_col]);
  for (int in_col = 0; in_col < in_dim; ++in_col) {
    sum += input[row * in_dim + in_col] *
           float16BitsToFloatDevice(weight[out_col * in_dim + in_col]);
  }
  output[idx] = sum;
}
...
__global__ void layerNormFloat16Kernel(const float* input,
                                       const std::uint16_t* gamma,
                                       const std::uint16_t* beta, float* output,
                                       int rows, int dim) {
  int row = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  for (int channel = 0; channel < dim; ++channel) {
    float normalized = (input[row * dim + channel] - mean) * inv_std;
    output[row * dim + channel] =
        normalized * float16BitsToFloatDevice(gamma[channel]) +
        float16BitsToFloatDevice(beta[channel]);
  }
}
...
int main() {
  ...
  const auto cpu_float16_probabilities =
      simple_transformer_reference::computeFloat16ReferenceProbabilities(demo);
  const auto cpu_float16_encoder_pooled =
      simple_transformer_reference::computeFloat16EncoderPooled(demo);
  ...
  const auto token_embedding_fp16 =
      simple_transformer_reference::quantizeToFloat16(token_embedding);
  const auto position_embedding_fp16 =
      simple_transformer_reference::quantizeToFloat16(position_embedding);
  const auto wq_fp16 = simple_transformer_reference::quantizeToFloat16(wq);
  const auto wk_fp16 = simple_transformer_reference::quantizeToFloat16(wk);
  const auto wv_fp16 = simple_transformer_reference::quantizeToFloat16(wv);
  const auto wo_fp16 = simple_transformer_reference::quantizeToFloat16(wo);
  const auto w1_fp16 = simple_transformer_reference::quantizeToFloat16(w1);
  const auto w2_fp16 = simple_transformer_reference::quantizeToFloat16(w2);
  const auto lm_head_fp16 =
      simple_transformer_reference::quantizeToFloat16(lm_head);
  const auto norm_gamma_fp16 =
      simple_transformer_reference::quantizeToFloat16(norm_gamma);
  const auto norm_beta_fp16 =
      simple_transformer_reference::quantizeToFloat16(norm_beta);
  const auto zero_model_bias_fp16 =
      simple_transformer_reference::quantizeToFloat16(zero_model_bias);
  const auto zero_hidden_bias_fp16 =
      simple_transformer_reference::quantizeToFloat16(zero_hidden_bias);
  const auto zero_vocab_bias_fp16 =
      simple_transformer_reference::quantizeToFloat16(zero_vocab_bias);
  ...
  std::uint16_t* d_token_embedding_fp16 = copyToDevice(token_embedding_fp16);
  std::uint16_t* d_position_embedding_fp16 = copyToDevice(position_embedding_fp16);
  std::uint16_t* d_wq_fp16 = copyToDevice(wq_fp16);
  std::uint16_t* d_wk_fp16 = copyToDevice(wk_fp16);
  std::uint16_t* d_wv_fp16 = copyToDevice(wv_fp16);
  std::uint16_t* d_wo_fp16 = copyToDevice(wo_fp16);
  std::uint16_t* d_w1_fp16 = copyToDevice(w1_fp16);
  std::uint16_t* d_w2_fp16 = copyToDevice(w2_fp16);
  std::uint16_t* d_lm_head_fp16 = copyToDevice(lm_head_fp16);
  std::uint16_t* d_norm_gamma_fp16 = copyToDevice(norm_gamma_fp16);
  std::uint16_t* d_norm_beta_fp16 = copyToDevice(norm_beta_fp16);
  std::uint16_t* d_zero_model_bias_fp16 = copyToDevice(zero_model_bias_fp16);
  std::uint16_t* d_zero_hidden_bias_fp16 = copyToDevice(zero_hidden_bias_fp16);
  std::uint16_t* d_zero_vocab_bias_fp16 = copyToDevice(zero_vocab_bias_fp16);
  ...
  embedFloat16Kernel<<<(model_values + threads - 1) / threads, threads>>>(
      d_tokens, d_token_embedding_fp16, d_position_embedding_fp16, d_x,
      simple_transformer_reference::kSeqLen,
      simple_transformer_reference::kModelDim);
  layerNormFloat16Kernel<<<1, simple_transformer_reference::kSeqLen>>>(
      d_x, d_norm_gamma_fp16, d_norm_beta_fp16, d_norm1,
      simple_transformer_reference::kSeqLen,
      simple_transformer_reference::kModelDim);
  linearFloat16Kernel<<<(model_values + threads - 1) / threads, threads>>>(
      d_norm1, d_wq_fp16, d_zero_model_bias_fp16, d_q,
      simple_transformer_reference::kSeqLen,
      simple_transformer_reference::kModelDim,
      simple_transformer_reference::kModelDim);
  linearFloat16Kernel<<<(model_values + threads - 1) / threads, threads>>>(
      d_norm1, d_wk_fp16, d_zero_model_bias_fp16, d_k,
      simple_transformer_reference::kSeqLen,
      simple_transformer_reference::kModelDim,
      simple_transformer_reference::kModelDim);
  linearFloat16Kernel<<<(model_values + threads - 1) / threads, threads>>>(
      d_norm1, d_wv_fp16, d_zero_model_bias_fp16, d_v,
      simple_transformer_reference::kSeqLen,
      simple_transformer_reference::kModelDim,
      simple_transformer_reference::kModelDim);
  attentionKernel<<<1, simple_transformer_reference::kSeqLen>>>(
      ...
      simple_transformer_reference::kModelDim);
  linearFloat16Kernel<<<(model_values + threads - 1) / threads, threads>>>(
      d_attn, d_wo_fp16, d_zero_model_bias_fp16, d_projected,
      simple_transformer_reference::kSeqLen,
      simple_transformer_reference::kModelDim,
      simple_transformer_reference::kModelDim);
  addKernel<<<(model_values + threads - 1) / threads, threads>>>(
      d_x, d_projected, d_x, model_values);
  layerNormFloat16Kernel<<<1, simple_transformer_reference::kSeqLen>>>(
      d_x, d_norm_gamma_fp16, d_norm_beta_fp16, d_norm2,
      simple_transformer_reference::kSeqLen,
      simple_transformer_reference::kModelDim);
  linearFloat16Kernel<<<(hidden_values + threads - 1) / threads, threads>>>(
      d_norm2, d_w1_fp16, d_zero_hidden_bias_fp16, d_hidden,
      simple_transformer_reference::kSeqLen,
      simple_transformer_reference::kModelDim,
      simple_transformer_reference::kHiddenDim);
  reluKernel<<<(hidden_values + threads - 1) / threads, threads>>>(d_hidden,
                                                                  hidden_values);
  linearFloat16Kernel<<<(model_values + threads - 1) / threads, threads>>>(
      d_hidden, d_w2_fp16, d_zero_model_bias_fp16, d_ffn,
      simple_transformer_reference::kSeqLen,
      simple_transformer_reference::kHiddenDim,
      simple_transformer_reference::kModelDim);
  addKernel<<<(model_values + threads - 1) / threads, threads>>>(d_x, d_ffn, d_x,
                                                                 model_values);
  linearFloat16Kernel<<<(vocab_values + threads - 1) / threads, threads>>>(
      d_x, d_lm_head_fp16, d_zero_vocab_bias_fp16, d_logits,
      simple_transformer_reference::kSeqLen,
      simple_transformer_reference::kModelDim,
      simple_transformer_reference::kVocabSize);
  softmaxRowsKernel<<<1, simple_transformer_reference::kSeqLen>>>(
  ...
  std::vector<float> gpu_float16_probabilities(
      simple_transformer_reference::kSeqLen *
      simple_transformer_reference::kVocabSize);
  CHECK_CUDA(cudaMemcpy(gpu_float16_probabilities.data(), d_probabilities,
                        gpu_float16_probabilities.size() * sizeof(float),
                        cudaMemcpyDeviceToHost));
  std::vector<float> gpu_float16_encoder_pooled(
      simple_transformer_reference::kModelDim);
  CHECK_CUDA(cudaMemcpy(gpu_float16_encoder_pooled.data(), d_encoder_pooled,
                        gpu_float16_encoder_pooled.size() * sizeof(float),
                        cudaMemcpyDeviceToHost));
  ...
  float float16_max_diff = simple_transformer_reference::maxAbsDiff(
      cpu_float16_probabilities, gpu_float16_probabilities);
  float float16_encoder_max_diff = simple_transformer_reference::maxAbsDiff(
      cpu_float16_encoder_pooled, gpu_float16_encoder_pooled);
  ...
  std::cout << "Max |CPU - GPU| FP16 probability diff: " << float16_max_diff
            << "\n";
  std::cout << "Max |CPU - GPU| FP16 encoder pooled diff: "
            << float16_encoder_max_diff << "\n";
  ...
  CHECK_CUDA(cudaFree(d_token_embedding_fp16));
  CHECK_CUDA(cudaFree(d_position_embedding_fp16));
  CHECK_CUDA(cudaFree(d_wq_fp16));
  CHECK_CUDA(cudaFree(d_wk_fp16));
  CHECK_CUDA(cudaFree(d_wv_fp16));
  CHECK_CUDA(cudaFree(d_wo_fp16));
  CHECK_CUDA(cudaFree(d_w1_fp16));
  CHECK_CUDA(cudaFree(d_w2_fp16));
  CHECK_CUDA(cudaFree(d_lm_head_fp16));
  CHECK_CUDA(cudaFree(d_norm_gamma_fp16));
  CHECK_CUDA(cudaFree(d_norm_beta_fp16));
  CHECK_CUDA(cudaFree(d_zero_model_bias_fp16));
  CHECK_CUDA(cudaFree(d_zero_hidden_bias_fp16));
  CHECK_CUDA(cudaFree(d_zero_vocab_bias_fp16));
  ...
  if (!std::isfinite(max_diff) || max_diff > 1.0e-4f ||
      ...
      !std::isfinite(float16_max_diff) || float16_max_diff > 1.0e-4f ||
      !std::isfinite(float16_encoder_max_diff) ||
      float16_encoder_max_diff > 1.0e-4f) {
