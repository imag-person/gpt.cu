// gpt.cu — single-file CUDA implementation of a GPT-style decoder-only transformer.
//
// Scope: forward pass / inference only. Weights are initialised with a fixed RNG
// seed so the build produces a deterministic demo output. The goal of this file
// is to be a readable reference, not a fast one: kernels are naive (no tensor
// cores, no fused attention, no kv cache) and use fp32 throughout. cuBLAS and
// cuDNN are intentionally avoided to keep this self-contained.
//
// Build:  nvcc -O3 -std=c++17 gpt.cu -o gpt
// Run:    ./gpt
//
// Model layout follows nanoGPT / GPT-2: pre-LayerNorm blocks, learned token and
// positional embeddings, multi-head causal self-attention, MLP with GELU, tied
// lm_head, final LayerNorm. Greedy decoding from a random prompt.
...
// Round-up integer division for kernel launch sizing.
...
//   wpe:    [block_size, C] positional embedding table
//   out:    [T, C]
...
// LayerNorm over the last dim. One block per (token) row, one thread per
// channel up to C. Uses shared memory for the partial sums.
//   x:   [N, C]
//   g,b: [C]
//   out: [N, C]
...
// row-wise softmax. One block per (head, query-position), threads cooperate
// over the key dimension.
//   q,k: [B=1, T, H, D]  laid out as [T, H, D]
...
__global__ void attn_value_kernel(const float *att, const float *v, float *out,
    ...
    // No causal guard needed: attn_scores_softmax_kernel already zeros att[h,t,j]
    // for j > t (via the -INFINITY before softmax), so those terms contribute 0.
    float acc = 0.f;
    for (int j = 0; j < T; ++j) {
...
// logits[v] = dot(x_last[:], wte[v, :]).
//   x_last: [C]
//   wte:    [V, C]
//   logits: [V]
__global__ void lm_head_kernel(const float *x_last, const float *wte,
                               float *logits, int V, int C) {
    int v = blockIdx.x * blockDim.x + threadIdx.x;
    if (v >= V) return;
    const float *row = wte + v * C;
    float acc = 0.f;
    for (int c = 0; c < C; ++c) acc += x_last[c] * row[c];
    logits[v] = acc;
}
...
// All per-layer weights, on the device.
struct Block {
    float *ln1_g, *ln1_b;       // [C]
...
struct GPT {
    ...
    float *lnf_g, *lnf_b;       // [C]
    // lm_head is tied to wte, so no separate buffer.
};

// Host helper: allocate device buffer of `n` floats and fill it with
// N(0, std) samples generated on host (deterministic, std::mt19937).
...
// Allocate a device buffer of `n` floats and zero it.
...
// Scratch buffers reused across blocks. All sizes are upper bounds for the
// largest T we'll ever pass through (= block_size).
struct Workspace {
    float *x;        // [T, C]    residual stream
    float *h;        // [T, C]    block input copy / temporary
    ...
    float *attn_out; // [T, C]
    float *ff1;      // [T, 4C]
    float *logits;   // [V]       last token logits only
};
...
static void forward(const GPT &m, Workspace &w, const int *tokens_d, int T) {
    ...
    // lm_head: tied with wte. wte is [V, C] row-major, so for the last
    // position logits[v] = sum_c wte[v, c] * last_row[c]. One thread per v.
    int threads = 256;
...
int main(int argc, char **argv) {
    // A small but non-trivial config. Sized so the demo runs in well under a
    // second on any modern GPU; bump these to scale up.
    Config cfg{};
    cfg.vocab_size = 1024;
    cfg.n_layer    = 4;
    cfg.n_head     = 4;
    ...
    std::printf("gpt.cu — device: %s (sm_%d%d)\n", prop.name,
    ...
    for (int step = 0; step < n_generate; ++step) {
        ...
        if (T > cfg.block_size) {
            // Truncate from the left to honour the context window.
