...
std::vector<float> expectedFloat16Probabilities() {
  return {
      0.08276045f, 0.08402198f, 0.08261459f, 0.08412381f, 0.08256393f,
      0.08412353f, 0.08261505f, 0.08402149f, 0.08276125f, 0.08383144f,
      0.08298387f, 0.08357857f, 0.08514546f, 0.08111124f, 0.08574308f,
      0.08069246f, 0.08601544f, 0.08060639f, 0.08592351f, 0.08086225f,
      0.08548101f, 0.08142933f, 0.08475053f, 0.08223934f, 0.08691721f,
      0.07932502f, 0.08703405f, 0.07945921f, 0.08662605f, 0.08006117f,
      0.08575228f, 0.08105980f, 0.08453958f, 0.08233599f, 0.08315716f,
      0.08373244f, 0.08273375f, 0.08413307f, 0.08222619f, 0.08458883f,
      0.08185966f, 0.08487593f, 0.08167996f, 0.08495544f, 0.08170903f,
      0.08481649f, 0.08194324f, 0.08447828f,
  };
}

std::vector<float> expectedFloat16EncoderPooled() {
  return {0.00673097f, 0.02041096f, 0.03121097f, 0.03817205f,
          0.03972718f, 0.03653558f, 0.02801573f, 0.01642990f};
}
...
int main() {
  ...
  const auto actual_float16_probabilities =
      simple_transformer_reference::computeFloat16ReferenceProbabilities(demo);
  const auto expected_float16_probabilities = expectedFloat16Probabilities();
  const auto actual_float16_encoder_pooled =
      simple_transformer_reference::computeFloat16EncoderPooled(demo);
  const auto expected_float16_encoder_pooled = expectedFloat16EncoderPooled();
  ...
  if (actual_float16_probabilities.size() != expected_float16_probabilities.size()) {
    std::cerr << "Unexpected FP16 probability size: "
              << actual_float16_probabilities.size() << " vs "
              << expected_float16_probabilities.size() << '\n';
    return 1;
  }
  if (actual_float16_encoder_pooled.size() !=
      expected_float16_encoder_pooled.size()) {
    std::cerr << "Unexpected FP16 encoder pooled size: "
              << actual_float16_encoder_pooled.size() << " vs "
              << expected_float16_encoder_pooled.size() << '\n';
    return 1;
  }
  ...
  const float float16_probability_max_diff =
      simple_transformer_reference::maxAbsDiff(actual_float16_probabilities,
                                               expected_float16_probabilities);
  const float float16_encoder_max_diff = simple_transformer_reference::maxAbsDiff(
      actual_float16_encoder_pooled, expected_float16_encoder_pooled);
  ...
  if (!std::isfinite(float16_probability_max_diff) ||
      float16_probability_max_diff > 1.0e-6f) {
    std::cerr << "Reference FP16 probability regression failed, max diff = "
              << float16_probability_max_diff << '\n';
    return 1;
  }
  if (!std::isfinite(float16_encoder_max_diff) ||
      float16_encoder_max_diff > 1.0e-6f) {
    std::cerr << "Reference FP16 encoder regression failed, max diff = "
  ...
  if (simple_transformer_reference::floatToFloat16Bits(1.0f) != 0x3c00u ||
      simple_transformer_reference::float16BitsToFloat(0x3c00u) != 1.0f) {
    std::cerr << "FP16 conversion helpers failed known-value check\n";
    return 1;
  }
  ...
  std::cout << "simple_transformer_reference_test passed, probability diff = "
            ...
            << ", FP16 probability diff = " << float16_probability_max_diff
            << ", FP16 encoder diff = " << float16_encoder_max_diff
            << '\n';
  return 0;
}
