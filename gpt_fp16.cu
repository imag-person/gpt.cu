// gpt_fp16.cu — fp16-storage variant of gpt.cu.
//
// Same model and same naive kernels as gpt.cu, but weights and activations
// are stored in __half (half the memory bandwidth of fp32). Kernels
// load fp16 -> fp32, accumulate in fp32, and store fp32 -> fp16 on the way
...
//
// Requires sm_53+ (Pascal) for __half.
// Build:  nvcc -O3 -std=c++17 -arch=sm_53 gpt_fp16.cu -o gpt_fp16
// Run:    ./gpt_fp16
...
__device__ __forceinline__ float    h2f(fp16 x) { return __half2float(x); }
__device__ __forceinline__ fp16     f2h(float x) { return __float2half(x); }
...
// fp32 -> fp16 conversion. Used during weight upload (host fp32 -> device fp16).
__global__ void f32_to_fp16_kernel(const float *src, fp16 *dst, int n) {
...
//   tokens: [T] int32
//   wte:    [V, C]    fp16
//   wpe:    [block_size, C]  fp16
//   out:    [T, C]    fp16
__global__ void embed_kernel(const int *tokens, const fp16 *wte,
                             const fp16 *wpe, fp16 *out,
                             int T, int C, int V) {
    int t = blockIdx.x;
    ...
    out[t * C + c] = f2h(h2f(wte[tok * C + c]) + h2f(wpe[t * C + c]));
}
...
//   g,b: [C]     fp16
//   out: [N, C]  fp16
__global__ void layernorm_kernel(const fp16 *x, const fp16 *g,
                                 const fp16 *b, fp16 *out,
                                 int N, int C, float eps) {
    int row = blockIdx.x;
    if (row >= N) return;
    extern __shared__ float smem[];

    const fp16 *xr = x + row * C;
    fp16 *or_ = out + row * C;

    // Two-pass: mean first, then variance over (x - mean)^2. Avoids the
    // E[x^2] - E[x]^2 cancellation when |x| is large relative to var.
    float local_sum = 0.f;
    for (int i = threadIdx.x; i < C; i += blockDim.x) local_sum += h2f(xr[i]);
    ...
    for (int i = threadIdx.x; i < C; i += blockDim.x) {
        float n = (h2f(xr[i]) - mean) * rstd;
        or_[i] = f2h(n * h2f(g[i]) + h2f(b[i]));
    }
}

// Naive matmul: out = x @ W + (bias ? bias : 0).
//   x:    [M, K]  fp16
//   W:    [K, N]  fp16  (row-major: W[k * N + n])
//   bias: [N]     fp16 or nullptr
//   out:  [M, N]  fp16
// Accumulate in fp32 then narrow.
__global__ void matmul_kernel(const fp16 *x, const fp16 *W, const fp16 *bias,
                              fp16 *out, int M, int N, int K) {
    int m = blockIdx.y * blockDim.y + threadIdx.y;
    int n = blockIdx.x * blockDim.x + threadIdx.x;
    if (m >= M || n >= N) return;
    float acc = 0.f;
    const fp16 *xr = x + m * K;
    for (int k = 0; k < K; ++k) acc += h2f(xr[k]) * h2f(W[k * N + n]);
    if (bias) acc += h2f(bias[n]);
    out[m * N + n] = f2h(acc);
}

// out[i] = a[i] + b[i]
__global__ void residual_kernel(const fp16 *a, const fp16 *b, fp16 *out, int N) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < N) out[i] = f2h(h2f(a[i]) + h2f(b[i]));
}

// GPT-2 / GELU (tanh approximation). In-place on fp16, computed in fp32.
__global__ void gelu_kernel(fp16 *x, int N) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= N) return;
    float v = h2f(x[i]);
    float c = 0.7978845608028654f; // sqrt(2/pi)
    float t = c * (v + 0.044715f * v * v * v);
    x[i] = f2h(0.5f * v * (1.f + tanhf(t)));
}



// Scaled dot-product attention scores with a causal mask, followed by a
// row-wise softmax. q,k stay in fp16; att stays in fp32 (softmax precision).
//   q,k: [T, H, D]  fp16
//   att: [H, T, T]  fp32
__global__ void attn_scores_softmax_kernel(const fp16 *q, const fp16 *k,
    ...
    const fp16 *qv = q + (i * H + h) * D;
    for (int j = threadIdx.x; j < T; j += blockDim.x) {
        if (j > i) {
            att[(h * T + i) * T + j] = -INFINITY;
        } else {
            const fp16 *kv = k + (j * H + h) * D;
            float s = 0.f;
            for (int d = 0; d < D; ++d) s += h2f(qv[d]) * h2f(kv[d]);
...
// out = att @ V, per head. att is fp32, v and out are fp16.
//   att: [H, T, T]  fp32
//   v:   [T, H, D]  fp16
//   out: [T, H, D]  fp16
__global__ void attn_value_kernel(const float *att, const fp16 *v, fp16 *out,
                                  int T, int H, int D) {
    int t = blockIdx.x;
    int h = blockIdx.y;
    int d = threadIdx.x;
    ...
    for (int j = 0; j < T; ++j) {
        ...
        acc += a * h2f(v[(j * H + h) * D + d]);
    }
    out[(t * H + h) * D + d] = f2h(acc);
}

// Split a [T, 3*C] qkv tensor into three [T, H, D] tensors.
__global__ void split_qkv_kernel(const fp16 *qkv, fp16 *q, fp16 *k, fp16 *v,
                                 int T, int C) {
    int t = blockIdx.x;
    int c = blockIdx.y * blockDim.x + threadIdx.x;
    if (t >= T || c >= C) return;
    const fp16 *row = qkv + t * 3 * C;
    q[t * C + c] = row[0 * C + c];
    k[t * C + c] = row[1 * C + c];
    v[t * C + c] = row[2 * C + c];
}

// logits[v] = dot(x_last[:], wte[v, :]). x_last and wte are fp16; logits fp32.
__global__ void lm_head_kernel(const fp16 *x_last, const fp16 *wte,
                               float *logits, int V, int C) {
    int v = blockIdx.x * blockDim.x + threadIdx.x;
    if (v >= V) return;
    const fp16 *row = wte + v * C;
    float acc = 0.f;
    for (int c = 0; c < C; ++c) acc += h2f(x_last[c]) * h2f(row[c]);
    logits[v] = acc;
}
...
// All per-layer weights, on the device, in fp16.
struct Block {
    fp16 *ln1_g, *ln1_b;        // [C]
    fp16 *qkv_w, *qkv_b;        // [C, 3C], [3C]
    fp16 *proj_w, *proj_b;      // [C, C],  [C]
    fp16 *ln2_g, *ln2_b;        // [C]
    fp16 *fc_w,  *fc_b;         // [C, 4C], [4C]
    fp16 *fcp_w, *fcp_b;        // [4C, C], [C]
};

struct GPT {
    Config cfg;
    fp16 *wte;                  // [V, C]
    fp16 *wpe;                  // [block_size, C]
    std::vector<Block> blocks;
    fp16 *lnf_g, *lnf_b;        // [C]
    // lm_head is tied to wte.
};

// Allocate a device fp16 buffer of n elements, fill it by uploading fp32
// values from host via a temporary fp32 device buffer + conversion kernel.
// (host -> device conversions for fp16 aren't trivially available, so the
// staging round-trip keeps things portable.)
static fp16 *upload_bf16(const std::vector<float> &host) {
    int n = (int)host.size();
    fp16 *d = nullptr;
    CUDA_CHECK(cudaMalloc(&d, n * sizeof(fp16)));
    ...
    int threads = 256;
    f32_to_fp16_kernel<<<cdiv(n, threads), threads>>>(staging, d, n);
    CUDA_CHECK(cudaFree(staging));
    return d;
}

static fp16 *alloc_init(std::mt19937 &rng, int n, float std) {
...
static fp16 *alloc_zeros_bf16(int n) {
    fp16 *d = nullptr;
    CUDA_CHECK(cudaMalloc(&d, n * sizeof(fp16)));
    // fp16 zero is bit-pattern 0, so memset(0) is valid.
    CUDA_CHECK(cudaMemset(d, 0, n * sizeof(fp16)));
    return d;
}

static fp16 *alloc_ones(int n) {
...
// Scratch buffers reused across blocks. Activations are fp16, except `att`
...
// Launch helpers.
static void launch_matmul(const fp16 *X, const fp16 *W, const fp16 *b,
                          fp16 *out, int M, int N, int K) {
    dim3 bs(16, 16);
...
static void launch_layernorm(const fp16 *x, const fp16 *g, const fp16 *bi,
                             fp16 *out, int N, int C) {
    int threads = 256;
    size_t shm  = threads * sizeof(float);
    layernorm_kernel<<<N, threads, shm>>>(x, g, bi, out, N, C, 1e-5f);
}

static void launch_residual(const fp16 *a, const fp16 *b, fp16 *out, int N) {
    int threads = 256;
...
static void block_forward(const Block &bl, const Config &cfg, Workspace &w,
    ...
    // --- attention ---
    CUDA_CHECK(cudaMemcpy(w.h, w.x, NC * sizeof(fp16), cudaMemcpyDeviceToDevice));
...
int main(int argc, char **argv) {
    ...
    std::printf("gpt_fp16.cu — device: %s (sm_%d%d)\n", prop.name,
                prop.major, prop.minor);
    if (prop.major < 5 || (prop.major == 5 && prop.minor < 3)) {
        std::fprintf(stderr,
            "warning: fp16 storage requires sm_53+; this device is sm_%d%d\n",
            prop.major, prop.minor);
    }
